...
from probabilistic_lib.functions import publish_lines, get_map, \
    get_dataset3_map, get_ekf_msgs, yaw_from_quaternion, angle_wrap, comp
from ekf_slam import EKF_SLAM


# ==============================================================================
class LocalizationNode(object):
    """Hold all ROS related transactions to use split&merge algorithm."""
    ...
    def odom_callback(self, msg):
        """
        Publish a tf based on the odometry of the robot.

        Also calculates the incremental odometry as seen from the robot frame.
        """
        # Save time
        ...
        if self.last_odom is not None and not self.new_odom:
            ...
            lastx = self.last_odom.pose.pose.position.x
            lasty = self.last_odom.pose.pose.position.y
            ...
            self.uk = np.array([deltax * np.cos(lyaw) + deltay * np.sin(lyaw),
                                - deltax * np.sin(lyaw) +
                                deltay * np.cos(lyaw),
                                angle_wrap(yaw - lyaw)])
    ...
    def iterate(self):
        ...
        if self.new_laser:
            ...
            Innovk_List, H_k_List, Rk_List, idx_not_associated = \
    ...
    def publish_results(self):
        ...
        publish_lines(self.map, self.pub_map_gt, frame='/world',
                      ns='gt_map', color=(0.3, 0.3, 0.3))

        odom, ellipse, trans, rot, room_map = get_ekf_msgs(self.ekf)

        publish_lines(room_map, self.pub_map, frame='/world',
                      ns='map', color=(0, 1, 0))
...
if __name__ == '__main__':
    ...
    node = LocalizationNode(x0=0.0, y0=0.0, theta0=0.0,

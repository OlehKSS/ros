...
from probabilistic_lib.functions import comp, get_polar_line, get_map
...
class EKF(object):
    ...
    #========================================================================
    def __init__(self, xinit, odom_lin_sigma, 
        ...
        self.map = get_map(xinit[0], xinit[1], xinit[2])
    ...
    def predict(self, uk):
        '''
        Implements the prediction equations of the EKF, saving the new
        robot state and uncertainty.
    ...
    def data_association(self, lines):
        '''
        Looks for closest correspondences between the provided measured
        lines and the map known a priori.
    ...
    def update_position(self, Hk_list, Vk_list, Sk_list, Rk_list) :
        '''
        Updates the position of the robot according to the given the 
        current position and the data association parameters. All input 
        lists have the same lenght.
        ...
        Input:
          ...
          Rk_list : list of 2x2 matrices (measurement uncertainty)
        '''
        # Compose matrices as single ones
        n = len(Hk_list)
        H = np.zeros((2*n, 3))
        v = np.zeros((2*n))
        S = np.zeros((2*n, 2*n))
    ...
    def jacobianH(self, lineworld, xk):
        '''
        Computes the jacobian of the get_polar_line function with
        respect to the robot state xk (done in pre-lab).
        '''
        # TODO: Program this function
        ################################################################
